...
def select_next_guess(
    ...
    *,
    ...
    optimal:
        ...
        effectively Knuth's original algorithm and can guarantee the minimal
        number of guesses for a given rule set.  For a four-digit decimal PIN
        it still needs up to five attempts, but that is provably optimal.  The
        exhaustive search is extremely slow as it requires scoring every
        possible guess against every candidate.  With ``False`` (the default) a
        deterministic sample is used to keep the solver responsive for longer
        pins.
    ...
    *,
    ...
    Args:
        ...
        optimal: when True, always evaluate the entire search space to select
            ...
            mirrors the strategy that yields the theoretically minimal number
            of guesses (five in the worst case for a four-digit decimal PIN).
    ...
    # Pick an initial guess.  The exhaustive strategy benefits from selecting a
    # minimax choice right from the start, whereas the heuristic mode sticks to
    # the simple patterned guess for speed.
    if optimal:
        guess = select_next_guess(candidates, all_codes, optimal=True)
    else:
